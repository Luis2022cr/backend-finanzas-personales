...
export const register = async (req: Request, res: Response): Promise<void> => {
    const { nombre, apellido, usuario, role_id, password, confirmPassword } = req.body;

    // Validar que todos los campos requeridos estén presentes
    if (!nombre || !apellido || !usuario || !role_id || !password || !confirmPassword) {
    ...
    try {
        ...
        await client.execute({
            sql: 'INSERT INTO usuarios (id, usuario, password, nombre, apellido, role_id) VALUES ( ?, ?, ?, ?, ?, ?, ?)',
            args: [id, usuario, hashedPassword, nombre, apellido, role_id]
        });
